...
exports.monthlyMail = function(email, data, name, month) {
  ...
  XlsxPopulate.fromFileAsync(path_dir + 'monthly_report.xlsx')
    .then(workbook => {
      ...
      return workbook.toFileAsync(path_dir + 'MonthlyReport_'+name+'_Thang'+month+'.xlsx');
    })
    .then(() => {
      var mailOptions = {
        from: 'Feedback Center of Quan Khu 5',
        ...
        attachments: [
          {
            path: path_dir + 'MonthlyReport_'+name+'_Thang'+month+'.xlsx'
          }
        ]
      };
      // send action by transporter
      smtpTransport.sendMail(mailOptions).then(success=>{
          fs.unlinkSync(path_dir + 'MonthlyReport_'+name+'_Thang'+month+'.xlsx');
      }).catch(function(err) {
